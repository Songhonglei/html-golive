...
TRANSLATIONS = {
    ...
    "arg.doctor.site": "检查单个已发布站点（manifest 与磁盘上的页面对比），而不是整体体检",
    "doctor.site.empty_ref": "--site 需要一个站点标识（slug 或 site_id），当前传入的是空值",
    "doctor.site.header": "🔍 站点检查：{ref}",
    "doctor.site.not_found": "没有匹配 '{ref}' 的站点 —— 用 `golive list` 确认",
    "doctor.site.unreadable": "已发布的页面读取失败：{error}",
    "doctor.site.no_manifest_store": "manifest 存储不可用：{error}",
    "doctor.site.size": "   大小：    {bytes} 字节",
    "doctor.site.published": "   发布于： {at}（golive {version}，来源 {source}）",
    "doctor.site.data_models": "   数据：    {models}",
    "doctor.site.manifest_missing": "还没有 manifest —— 0.9.0 之前发布的站点会在下次发布时补上",
    "doctor.site.hash_match": "磁盘上的页面与上次发布写入的一致",
    "doctor.site.hash_mismatch": "磁盘上的页面与上次发布写入的不一致（记录 {stored}…，实际 {live}…）—— 可能在 golive 之外被改过",
    "doctor.site.layers_match": "注入层与记录一致：{layers}",
    "doctor.site.layers_missing": "记录里有但页面中缺失的层：{layers}",
    "doctor.site.layers_extra": "页面中有但记录里没有的层：{layers}",
    "doctor.site.policy_watermark_ok": "policy 要求水印，页面也有",
    "doctor.site.policy_watermark_absent": "policy 要求水印但页面没有 —— 重新发布，或者如果不再需要就清掉 policy",
    "doctor.site.reports_only": "   （doctor 只报告，不会改动已发布的页面或其记录）",
    "doctor.site.healthy": "✅ 该站点状态一致",
    "doctor.healthy": "✅ 环境健康。",
    ...
    "serve.app.token_env_typo": "\u26a0  \u5df2\u8bbe\u7f6e {wrong}\uff0c\u4f46 golive \u4e0d\u8bfb\u53d6\u8be5\u53d8\u91cf \u2014 \u8bf7\u6539\u7528 {right}\u3002\u5f53\u524d\u670d\u52a1\u5c06\u5728\u300c\u65e0\u9274\u6743\u300d\u4e0b\u542f\u52a8\uff1aloopback \u4e0a\u7684\u4efb\u4f55\u672c\u673a\u8fdb\u7a0b\u90fd\u88ab\u5f53\u4f5c superadmin\u3002",
    ...
    "scanner.rule.secret_credential": "机密凭证（强特征）",
    "scanner.rule.credential_noun": "凭证相关名词（弱特征）",
    "scanner.rule.personal_info": "个人信息",
    "scanner.rule.phone_cn": "手机号（11位）",
    "scanner.rule.id_card_cn": "身份证号（18位）",
    "scanner.rule.jdbc_dsn": "JDBC连接串",
    "scanner.rule.db_dsn": "数据库连接串",
    "scanner.rule.dsn_inline_password": "连接串含内嵌口令",
    "scanner.rule.credential_assignment": "凭证赋值（含变量声明）",
    "scanner.rule.bearer_token": "Bearer 令牌值",
    "scanner.rule.jwt": "JWT",
    "scanner.rule.aws_access_key": "AWS AccessKey",
    "scanner.rule.cloud_access_key_id": "云厂商 AccessKey ID",
    "scanner.rule.cloud_ak_sk_assignment": "云平台 AK/SK 赋值",
    "scanner.rule.openai_style_key": "OpenAI 风格 key",
    "scanner.rule.github_token": "GitHub token",
    "scanner.rule.github_token_fine": "GitHub 细粒度 token",
    "scanner.rule.pypi_token": "PyPI token",
    "scanner.rule.slack_token": "Slack token",
    "scanner.rule.google_api_key": "Google API key",
    "scanner.rule.private_key_header": "私钥文件头",
